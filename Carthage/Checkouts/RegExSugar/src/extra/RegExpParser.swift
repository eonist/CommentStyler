...
public class RegExpParser {
    ...
    */
    enum Pattern {
        ...
        static let contentBetweenTitleTags = "(?<=<[tT][iI][tT][lL][eE]>).*(?=<\\/[tT][iI][tT][lL][eE]>)"
    ...
    static func computerDate(_ usDate: String) -> String {
        ...
        usDate.matches(pattern).forEach {
         let month: String = $0.value(usDate, key: 1)
         let day: String = $0.value(usDate, key: 2)
         let year: String = $0.value(usDate, key: 3)
    ...
    static func lowerCaseWordsWithExecution(_ input: String) -> [String] {
      ...
      return RegExp.matches(input, pattern: pattern).map {
//            Swift.print("match.numberOfRanges: " + "\($0.numberOfRanges)")/*The first item is the entire match*/
         let word: String = $0.value(input, key: 1)/*capturing group 1*/
            return word
    ...
    /**
     * Returns each word with a hexadecimal rgb value from PARAM input
     * ## EXAMPLES:
   ...
   static func cssProperties(_ input: String) -> [(name: String, value: String)] {
      let pattern = "(\\w*?)\\:(.*?)\\;"//assigns the name and value to an object (Associative) // :Fixme: (the dot in the end part could possibly be replaced by [.^\;] test this)
      return input.matches(pattern).map {//Loops through the pattern
         let name = $0.value(input, key: 1)/*capturing group 1*/
         let value = $0.value(input, key: 2)/*capturing group 2*/
